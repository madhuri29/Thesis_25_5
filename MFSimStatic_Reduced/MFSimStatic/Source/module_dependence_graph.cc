...
#include "../Headers/module_dependence_graph.h"
...
void ModuleDependenceGraph::RecursiveComponentLabel(ModuleDependenceVertex *v, int componentNum)
{
	if (v->utilNum < 0)
	{
		...
		for (int p = 0; p < v->preds.size(); p++)
			RecursiveComponentLabel(v->preds.at(p), componentNum);
		for (int s = 0; s < v->succs.size(); s++)
...
/////////////////////////////////////////////////////////////
// Sorts the strongly connected components via a topological
...
void ModuleDependenceGraph::RevTopSortSCCs()
{
	for (int c = 0; c < connectedComponents.size(); c++)
	{
		...
		for (int i = 0; i < cc->size(); i++)
		{
			...
			for (int j = 0; j < node->succs.size(); j++)
		...
		for (unsigned i = 0; i < cc->size(); i++)
		{
			...
			{
				ModuleDependenceVertex *c = n->succs.at(j);
				{
					stringstream msg;
					msg << "ERROR. Cycle detected: Node " << n->utilNum << " (" << n->operation->GetName() << ") points to higher/equal node, Node " << c->utilNum << " (" << c->operation->GetName() << ")" << endl;
					claim((c->utilNum > n->utilNum) && n->utilNum != 0, &msg);
				}
...
/////////////////////////////////////////////////////////////
void ModuleDependenceGraph::RecursiveSccSearch(ModuleDependenceVertex *v)
{
    ...
    for (int i = 0; i < v->succs.size(); i++)
    {
    	...
    	if (w->utilNum < 0)
    		RecursiveSccSearch(w);
    	else if (w->inCurrCycle != true)
...
void ModuleDependenceGraph::FindAndResolveDependencies()
{
	...
	for (int i = 0; i < connectedComponents.size(); i++)
	{
		...
		for (int j = 0; j < cc->size(); j++)
		{
			...
			v->inCurrCycle = false; // Used to tell if assigned to a cycle already
			v->utilNum = -1; // preorder number for this search
		}

		for (int j = 0; j < cc->size(); j++)
			RecursiveSccSearch(cc->at(j));
	}
	...
	if (stronglyConnComps.size() > 0)
	{
		...
		for (int i = 0; i < stronglyConnComps.size(); i++)
		{
			cout << "C" << i << ": ";
			for (int j  = 0; j < stronglyConnComps.at(i)->size(); j++)
			{
		...
		claim(false, "Cycle-recovery has not yet been implemented because no cases arose in the initial benchmark set.");
	}
}
...
void ModuleDependenceGraph::PrintDependencies()
{
	...
	for (int i = 0; i < vertices.size(); i++)
	{
		...
		if (f->operation->GetType() == DISPENSE || f->operation->GetType() == OUTPUT)
		{
		...
		for (int s = 0; s < f->succs.size(); s++)
		{
			...
			else if (t->operation->GetType() == SPLIT)
			{
...
void ModuleDependenceGraph::PrintConnectedComponents()
{
	...
	for (int i = 0; i < connectedComponents.size(); i++)
	{
		...
		cout << endl;
		cout << "\tCC_Heads" << i << ": ";
		for (int j = 0; j < ccHeads.at(i)->size(); j++)
		{
		...
		cout << endl;
		cout << "\tCC_Tails" << i << ": ";
		for (int j = 0; j < ccTails.at(i)->size(); j++)
		{
...
bool ModuleDependenceGraph::ModuleLocationsMatch(AssayNode *n1, AssayNode *n2)
{
	...
	if (n1->GetType() == SPLIT && n2->GetType() == SPLIT)
	{
		ReconfigModule *srm0 = n1->GetChildren().at(0)->GetReconfigMod(); // Storage-module child
		ReconfigModule *srm1 = n1->GetChildren().at(1)->GetReconfigMod();
...
void ModuleDependenceGraph::AddDependency(AssayNode *from, AssayNode *to)
{
	...
	if (from == NULL || to == NULL || (to->GetType() == STORAGE && to->GetParents().front()->GetType() == SPLIT))
	{
