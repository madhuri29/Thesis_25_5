...
unsigned long long ListScheduler::schedule(DmfbArch *arch, DAG *dag)
{
	...
	while(moreNodesToSchedule())
	{
		...
		for (int i = 0; i < finishedOps.size(); i++)
		...
		for (; it != candidateOps->end(); it++)
		{
					...
					(unfinishedOps->size() + 1) + numStorageModules <= numModules)
			{
				...
				if ((n->type == SPLIT || n->type == MIX) && (availRes[BASIC_RES] + availRes[D_RES] + availRes[H_RES] + availRes[DH_RES]) > 0)
				{
					canSchedule = true;
				...
				if (canSchedule)
				{
					...
					for (int c = 0; c < n->GetChildren().size(); c++)
					{
						...
						for (int p = 0; p < child->GetParents().size(); p++)
						{
