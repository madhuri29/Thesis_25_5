...
void PathScheduler::setActiveAtTS(IoResource *input, unsigned long long ts)
{
	...
	for (int i = ts - input->durationInTS; i < ts; i++)
	{
...
unsigned long long PathScheduler::schedule(DmfbArch *arch, DAG *dag)
{
	...
	for (int i = 0; i < dag->allNodes.size(); i++)// Sort nodes by priority so we address the nodes with the least splits first
	...
	while (moreNodesToSchedule())
	{
		...
		if (mustRestartPathLater)
		{ 	// Throw away path
			for (int pi = 0; pi < pathIns.size(); pi++)
		...
		{ 	// Submit path
			...
			for (int po = 0; po < pathOuts.size(); po++)
			{
			...
			for (int pi = 0; pi < path.size(); pi++)
			{
				...
				for (int pc = 0; pc < pathNode->GetChildren().size(); pc++)
				{ // Add to indefStorage for the children
					...
					{
						if (pathNode->GetType() == SPLIT && cNode->GetType() != OUTPUT)
				...
				for (int pp = 0; pp < pathNode->GetParents().size(); pp++)
		...
		//dag->OutputGraphFile("partialPathSched", true, true); // DTG DEBUG
	...
	for (int i = 0; i < dag->allNodes.size(); i++)
	{
		...
		for (int j = 0; j < storeInsert.size(); j++)
